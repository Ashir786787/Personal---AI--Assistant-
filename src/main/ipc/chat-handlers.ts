...
async function streamResponse(
  ...
  while (true) {
    ...
    let full = ''
    try {
      for await (const chunk of provider.stream({ turns, apiKey, signal: controller.signal })) {
        full += chunk
      ...
      if (canFailOver && err.status === 429) {
        router.markRateLimited(err.provider)
        log('warn', 'chat', `${err.provider} rate-limited, failing over`)
        continue
      }
      throw err
    }
